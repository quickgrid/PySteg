...
class StegAppFrameSplitter(wx.Frame):
    """
    ...
    def __init__(self, *args, **kw):
        ...
        super(StegAppFrameSplitter, self).__init__(*args, **kw)
        ...
        self.file_picker = wx.FilePickerCtrl(self.pnl, message="Select Compressed File", size=wx.DefaultSize,
                                             style=wx.FLP_DEFAULT_STYLE,
                                             wildcard="Compressed (*.zip,*.rar,*.7z)|*.zip;*.rar;*.7z")
        ...
        text = wx.StaticText(self.pnl, label="Enter Resolution X and Y")
        sizer.Add(text, pos=(3, 0), flag=wx.TOP | wx.LEFT | wx.BOTTOM | wx.RIGHT, border=5)

        bs = wx.BoxSizer(wx.HORIZONTAL)
        self.t1 = wx.TextCtrl(self.pnl, style=wx.TE_CENTER, value="1920")
        bs.Add(self.t1, 1, wx.EXPAND)
        self.t2 = wx.TextCtrl(self.pnl, style=wx.TE_CENTER, value="1080")
        bs.Add(self.t2, 1, wx.EXPAND)
        sizer.Add(bs, pos=(3, 1), flag=wx.TOP | wx.LEFT)

        buttonOk = wx.Button(self.pnl, label="Open Selected Folder", id=1)
        buttonClose = wx.Button(self.pnl, label="Open Joiner", id=2)
        sizer.Add(buttonOk, pos=(6, 3))
        sizer.Add(buttonClose, pos=(6, 4), flag=wx.RIGHT | wx.BOTTOM, border=5)
    ...
    def makeMenuBar(self):
        ...
        tool3 = self.toolbar.AddTool(3, 'Split', wx.Bitmap('3d_img3.png'))
        tool4 = self.toolbar.AddTool(4, 'Convert', wx.Bitmap('3d_img4.png'))
        self.toolbar.Realize()
        self.Bind(wx.EVT_TOOL, self.OnClear, tool1)
        self.Bind(wx.EVT_TOOL, self.OnInfo, tool2)
        self.Bind(wx.EVT_TOOL, self.OnSplit, tool3)
        self.Bind(wx.EVT_TOOL, self.OnConvert, tool4)
    ...
    def OnSplit(self, event):
        zti_instance = zti3.ZipToImage3(self, self.file_picker.GetPath(), width=self.t1.GetValue(), height=self.t2.GetValue())
        self.setZtiInstance(zti_instance)
        # print(zti_instance.printDir())
        # print(zti_instance.getResolution())
        # print(zti_instance.folder_directory)
        #self.thread_pool_executor = futures.ThreadPoolExecutor(max_workers=1)
        #self.thread_pool_executor.submit(self.blocking_code2, zti_instance)
        self.blocking_code2(zti_instance)

        #with futures.ThreadPoolExecutor(max_workers=5) as executor:
        #    executor.submit(self.blocking_code2, zti_instance)
    ...
    def OnConvert(self, event):
        # zti_instance = zti3.ZipToImage3(self, self.file_picker.GetPath(), self.t1.GetValue(), self.t2.GetValue())
        ...
        #self.blocking_code3(self.getZtiInstance())

        #with futures.ThreadPoolExecutor(max_workers=1) as executor:
        #    executor.submit(self.blocking_code3, self.getZtiInstance())
        #futures.as_completed(executor)
        print("DONE JOIN")
    ...
    def blocking_code2(self, zti_instance):
        zti_instance.splitFile()
        #wx.CallAfter(self.set_status_text, 'Running')
        #if zti_instance.splitFile() == 0:
    ...
    def blocking_code3(self, zti_instance):
        ...
        wx.CallAfter(self.set_status_text, 'Running')
        val = zti_instance.convertToImage()
        print(val)
        if val == 0:
            print("SHUT DOWN")
        else:
    ...
    def button_press(self, event):
        ...
        if Id == 1:
            ...
            print("OPEN SPLIITER PRESSED")
            # thread_pool_executor.submit(self.blocking_code)
            self.blocking_code()

    def blocking_code(self):
        width, height = wx.GetDisplaySize()
        secondWindow = safp.StegAppFrameJoiner(self.pnl, title='File Joiner',
                                               pos=(width / 2 - width / 4 + 50, height / 2 - height / 4 + 50))
        # secondWindow.Center()
        secondWindow.SetSize(width / 2, height / 2)
        secondWindow.Show()
